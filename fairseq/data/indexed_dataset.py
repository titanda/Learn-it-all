...
class IndexedDataset(torch.utils.data.Dataset):
    ...
    def read_index(self, path):
        with open(index_file_path(path), 'r') as f:
            ...
            for line in all_lines:
                line = line.replace("\n","")
                if count==0:
                    self.element_size = int(line)
                elif count==1:
                    self.dtype = dtypes[int(line)]
                elif count==2:
                    self.size = int(line)
                    self.s = self.size
                elif count==3:
                    a = np.empty(self.s+1, dtype=np.int64)
                    line = line.strip()
                    ele = line.split(" ")
                    ...
                    a = np.empty(self.s, dtype=np.int64)
                    line = line.strip()
                    ele = line.split(" ")
                    for i,e in enumerate(ele):
                        a[i] = e
