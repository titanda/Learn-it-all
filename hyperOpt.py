...
def validate(args, trainer, task, epoch_itr, subsets):
    ...
    for subset in subsets:
        ...
        for sample in progress:
            ...
            #pdb.set_trace()
                ...
                #pdb.set_trace()
            if predict is None:
                predict = log_output['predict'][0]
                target = log_output['target'][0]
                true_prob = log_output['true_prob'][0]
            else:
                predict = torch.cat((predict,log_output['predict'][0]),0)
                target = torch.cat((target,log_output['target'][0]),0)
                true_prob = torch.cat((true_prob,log_output['true_prob'][0]),0)
            for k, v in log_output.items():
                if k in ['loss', 'nll_loss', 'ntokens', 'nsentences', 'sample_size']:
                    continue
                #extra_meters[k].update(v)
    ...
    roc_auc = roc_auc_score(target, true_prob)
